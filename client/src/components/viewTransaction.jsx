...
import DialogWrapper from "../components/wrappers/dialog-wrapper.jsx";
            ...
            <DialogPanel className="w-full max-w-md transform overflow-hidden rounded-2xl bg-white dark:bg-slate-900 p-6 text-left">
                ...
                <div className="mt-10 mb-3 flex justify-between">
                    <p className="text-black dark:text-gray-400 text-2xl font-bold">
                        <span className={`${data?.type === "income" ? "text-emerald-600":"text-red-600"} font-bold mgl-1`}>{data?.type === "income" ? "+" : "-"}</span>{" "}
                        {formatCurrency(data?.amount)}
                    </p>
                    <button type="button" className='rounded-md outline-none bg-violet-800 px-4 py-2 text-sm font-medium text-white' onClick={closeModal}>
                        Close
                    </button>
                </div>
            </DialogPanel>
